 ...
 */
  ...
  static async findAll(options = {}) {
    let { attributes = ['*'], exclude = [], where = {}, limit, offset, order, group } = options;
   ...
   * @returns {Promise<{ instance: MappifyModel, created: boolean }>} An object containing the instance and a boolean indicating if the record was created.
  ...
  static async findAndCountAll(options = {}) {
    var count = await query(`SELECT COUNT(*) AS count FROM ${this.tableName}`);
    var result = await this.findAll(options);
    if (result) {
      return { count: count[0].count, rows: result };
    }
    throw new Error('No record found');
  }
